...
#merge データフレームの結合　結合する列を指定する場合はON
...
df_mean_line = df_mean_line[["集計年","全国_一人当たり賃金（万円）","一人当たり賃金（万円）"]]#表示する列を絞る
df_mean_line = df_mean_line.set_index("集計年")#集計年をインデックス列にする
st.line_chart(df_mean_line)

#バブルチャート
st.header("■年齢階級別の全国一人あたり平均賃金（万円）")

df_mean_bub = df_pref_ind[df_pref_ind["年齢"] != "年齢計"]

fig = px.scatter(df_mean_bub,
                 x="一人当たり賃金（万円）",
                 y="年間賞与その他特別給与額（万円）",
                 range_x=(150,700),
                 range_y=(0,150),
